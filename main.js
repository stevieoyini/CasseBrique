...
let bricks = [
  { color: "#333333", numberCollisions: 1 },
  { color: "#00FF00", numberCollisions: 2 },
  { color: "#333333", numberCollisions: 1 },
  { color: "#00FF00", numberCollisions: 2 },
  { color: "#333333", numberCollisions: 1 },
  { color: "#00FF00", numberCollisions: 2 },
...
function detectCollisions() {
  ...
  if (ball.y + ball.radius >= game.canvasDom.height) game.gameOver = true;

  // Si la balle arrive en haut du canvas elle rebondit
  if (ball.y - ball.radius <= 0) ball.direction.y *= -1;

  // Si la balle arrive à droite ou à gauche du canvas elle rebondit
  if (ball.x <= 0 || ball.x + ball.radius >= game.width) ball.direction.x *= -1;

  // Si la balle touche le plateau elle rebondit
  if (
    (ball.y == paddle.y || ball.y + ball.speed == paddle.y + ball.speed) &&
    ball.x >= paddle.x &&
    ...
    if (
      ...
      ball.x + ball.radius >= paddle.x + (paddle.width * 2) / 3 ||
      ball.x - ball.radius >= paddle.x + (paddle.width * 2) / 3
    )
      ball.direction.x = 1;
  ...
  bricks.forEach((brick) => {
    if (brick.numberCollisions > 0) {
      ...
      if (
        (ball.direction.y == -1 && ball.y - ball.radius == brick.y + 20) ||
        (ball.direction.y == 1 && ball.y + ball.radius == brick.y)
      ) {
...
function displayBricks() {
  ...
  if (displayingBrick == 0) game.win = true;
}
...
function displayGameOver() {
  ...
  game.ctx.fillText(
    "Game Over",
    game.width / 2 - txtMetrics.width / 2,
    game.height / 2
  );
}
...
function keyboardEvent(e) {
  ...
  switch (e.code) {
    case "ArrowRight":
      if (e.type == "keydown") paddle.direction = 1;
    ...
    case "Space":
      if (e.type == "keydown") {
        if (game.start == false) game.start = true;
        else if (game.gameOver == true) {
          game.gameOver = false;
          game.start = false;
          initPositions();
        } else game.pause = !game.pause;
      }
  }
}
